...
public class CloudManager : MonoBehaviour
{
	...
	void Init(RenderTexture src)
	{
		...
		CreateMaterial(ref cloudMaterial, cloudShader);
		CreateMaterial(ref compositeMaterial, compositeShader);
		...
		ComputeHelper.CreateRenderTexture(ref cloudRender, cloudRenderWidth, cloudRenderHeight, src.filterMode, src.graphicsFormat);
		cloudRender.wrapMode = TextureWrapMode.Clamp;
		if (noiseGenerator == null)
		{
			noiseGenerator = FindObjectOfType<NoiseGenerator>();
		}
		sunTransform = GameObject.Find("Test Sun").transform;
	}
    ...
    public void Render(RenderTexture src, RenderTexture dest)
	{
		...
		cloudMaterial.SetVector("dirToSun", -sunTransform.forward);
	...
	public void CreateMaterial(ref Material material, Shader shader)
	{
