...
public class GaussianBlur
{
	...
	{
		if (halfBlurSize != currentHalfBlurSize || sigma != currentSigma)
		{
	...
	public void Blur(RenderTexture source, RenderTexture target, int halfBlurSize = 8, float sigma = 5)
	{
		...
		ComputeHelper.Run(blurCompute, source.width, source.height, kernelIndex: 0);
		ComputeHelper.Run(blurCompute, source.width, source.height, kernelIndex: 1);
